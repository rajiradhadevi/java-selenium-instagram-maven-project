...
public class DriverHelper {
    ...
    public static boolean AssertPageUrlContains(String givenUrl) throws Exception {

        WebDriver driver = AppConfiguration.driver;
        String currUrl = driver.getCurrentUrl();
        boolean result = currUrl.toLowerCase().contains(givenUrl.toLowerCase());
        if(!result) {
            throw new Exception(String.format("%s does not contain %s", currUrl, givenUrl));
        }
        return result;
    }
}
